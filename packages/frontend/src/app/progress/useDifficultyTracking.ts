...
import { adjustDifficulty, DifficultyNotification } from './adjustDifficulty'
    ...
    if (adjustmentNotification) {
      ...
      if (newDifficulty !== currentDifficulty) {
        ...
        if (!updatedUserData.progress[subject][conceptId]) {
          updatedUserData.progress[subject][conceptId] = createEmptyConceptStats()
        }
  ...
  const setDifficulty = useCallback(
    (newDifficulty: Difficulty) => {
      ...
      if (!updatedProgress[subject][conceptId]) {
        updatedProgress[subject][conceptId] = createEmptyConceptStats()
      }
...
function createEmptyConceptStats(): ConceptStats {
  return {
    attempts: [],
    aggregate: {
      totalAttempts: 0,
