...
function checkSingleConcept(conceptPath: string): CheckResult {
  ...
  try {
    ...
    if (!conceptId) {
      console.error(`${colors.red}${symbols.fail} Error: Concept has no 'id' in frontmatter${colors.reset}`);
      process.exit(1);
    }
...
function printSubjectResult(subject: string, result: CheckResult): boolean {
  ...
  const hasWarnings =
    result.orphanedTranslations.length > 0 ||
    result.crossLanguageInconsistencies.length > 0;
  ...
  if (result.crossLanguageInconsistencies.length > 0) {
    ...
    for (const item of result.crossLanguageInconsistencies) {
      console.log(`    - "${item.key}" exists in ${item.inLang} but missing from ${item.missingFrom}`);
    }
  }
...
function printConceptResult(result: CheckResult): boolean {
  ...
  console.log(`${colors.red}${symbols.fail} FAIL${colors.reset} - ${result.missingTranslations.length} issue(s)\n`);
...
async function main() {
  ...
  if (input === '--all') {
    ...
    for (const subject of subjects) {
      ...
      const warnings = result.orphanedTranslations.length + result.crossLanguageInconsistencies.length;
