...
describe("Arithmetic Routes", () => {
  ...
  test("GET /api/tasks/arithmetic should return a valid task", async () => {
  ...
  test("GET /api/tasks/arithmetic should handle service errors", async () => {
    ...
    await request
      ...
      .expect(res => {
        ...
        expect(res.body.error).toBe("Failed to generate arithmetic task");
      });
  });
}); 