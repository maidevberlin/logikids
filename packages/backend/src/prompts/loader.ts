...
export class PromptLoader {
  ...
  async loadConcepts(subjectId: string): Promise<Concept[]> {
    ...
    const officialConcepts = await this.loadConceptsFromDirectory(officialPath, 'curriculum')
    ...
    const customConcepts = await this.loadConceptsFromDirectory(customPath, 'custom')
  ...
  enableHotReload(): void {
    ...
    this.watcher = chokidar.watch([`${this.promptsDir}/**/*.md`, `${this.contentDir}/**/*.md`], {
      ignoreInitial: true,
      persistent: true,
    })
  ...
  private async getConceptFiles(subjectDir: string): Promise<string[]> {
    try {
      ...
      return files.filter((file) => file.endsWith('.md') && file !== 'base.md')
    } catch (error) {
      return []
    }
  }
